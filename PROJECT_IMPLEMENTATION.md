# Code Tutor Api — Standalone Real GUI Implementation
...
Default URL: `http://127.0.0.1:9109`

## What is inside this project folder

- `app/` — FastAPI backend for this project.
- `static/` — elegant browser GUI.
- `plugins/code-tutor-api.json` — this project’s own feature/customization/input schema.
...
- `programming_language` — programming language (select)
- `learner_level` — learner level (select)
- `curriculum` — curriculum (text)
- `hint_style` — hint style (select)
- `rubric` — rubric (text)
- `explanation_depth` — explanation depth (text)
- `exercise_difficulty` — exercise difficulty (text)
...
- `student_code` — Student code (text) required
