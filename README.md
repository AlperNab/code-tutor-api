...
**Domain:** `Education / Coding` · **Suite:** `Developer Productivity Suite` · **Accent:** `#60a5fa`
...
**Code Tutor Api** is a standalone, browser-based AI workflow app for **Education / Coding**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Student code → explanation, diagnosis, next exercise.
...
|---|---:|:---:|---|---|
| `student_code`<br>Student code | text | Yes | — | Affects input: Student code. |
...
| `programming_language`<br>programming language | select | No | English | Affects customization: programming language. |
| `learner_level`<br>learner level | select | No | — | Affects customization: learner level. |
| `curriculum`<br>curriculum | text | No | — | Affects customization: curriculum. |
| `hint_style`<br>hint style | select | No | — | Affects customization: hint style. |
| `rubric`<br>rubric | text | No | — | Affects customization: rubric. |
| `explanation_depth`<br>explanation depth | text | No | — | Affects customization: explanation depth. |
| `exercise_difficulty`<br>exercise difficulty | text | No | — | Affects customization: exercise difficulty. |
...
- **programming language**: English, Arabic, Egyptian Arabic, French, German, Spanish
...
uvicorn app.main:app --host 127.0.0.1 --port 9109
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9109`. |
...
```bash
curl http://127.0.0.1:9109/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9109/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9109 uvicorn app.main:app --host 0.0.0.0 --port 9109
```
...
- `plugins/code-tutor-api.json` for plugin metadata.
